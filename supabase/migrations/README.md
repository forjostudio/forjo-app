...
- **026** — ⚠🔒 **Seguridad (crítico)**: vista pública acotada `public_businesses` +
  `DROP` de la policy abierta `public read businesses`. Sin esta migración el rol `anon`
  puede leer la fila entera de `businesses` (incluidos `mp_access_token`, `resend_api_key`,
  `recaptcha_secret_key`, `google_refresh_token`) → fuga de credenciales entre tenants.
  Acoplada al deploy: el código nuevo lee `public_businesses` en `/[slug]`. **Correr ya.**
