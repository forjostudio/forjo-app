...
/* accent palettes available per theme (Forjo's live in theme.css) */
const THEME_PALETTES = {
  forjo: PALETTES,
  modern: [
    { id:'indigo',  name:'Índigo',    meta:'Por defecto', sw:['#5b6ef5','#1b2334','#1fbf91'] },
    { id:'emerald', name:'Esmeralda', meta:'Fresco',      sw:['#10b981','#1b2334','#5b6ef5'] },
    { id:'violet',  name:'Violeta',   meta:'Creativo',    sw:['#8b5cf6','#1b2334','#f4b740'] },
    { id:'rose',    name:'Rosa',      meta:'Vibrante',    sw:['#f43f5e','#1b2334','#5b6ef5'] },
    { id:'amber',   name:'Ámbar',     meta:'Cálido',      sw:['#f59e0b','#1b2334','#10b981'] },
  ],
  spa: [
    { id:'sage',     name:'Salvia',    meta:'Por defecto', sw:['#7e9b82','#b79aa0','#cdb083'] },
    { id:'mauve',    name:'Malva',     meta:'Floral',      sw:['#b08a92','#cdb083','#7e9b82'] },
    { id:'clay',     name:'Arcilla',   meta:'Terroso',     sw:['#bb8b6b','#9bb0a0','#cdb083'] },
    { id:'ocean',    name:'Océano',    meta:'Sereno',      sw:['#7e93a6','#a9b8a0','#cdb083'] },
    { id:'lavender', name:'Lavanda',   meta:'Calmo',       sw:['#9d8fb0','#c6b59a','#7e9b82'] },
  ],
  cyber: [
    { id:'cyan',    name:'Cian',     meta:'Por defecto', sw:['#00e5ff','#ff2e7e','#3dff9e'], glow:true },
    { id:'magenta', name:'Magenta',  meta:'Synthwave',   sw:['#ff2e7e','#00e5ff','#f5e663'], glow:true },
    { id:'lime',    name:'Lima',     meta:'Matrix',      sw:['#3dff9e','#b06cff','#00e5ff'], glow:true },
    { id:'purple',  name:'Púrpura',  meta:'Vapor',       sw:['#b06cff','#00e5ff','#ff2e7e'], glow:true },
    { id:'amber',   name:'Ámbar',    meta:'Reactor',     sw:['#ffd23d','#ff2e7e','#00e5ff'], glow:true },
  ],
};
const THEME_DEFAULT_PAL = { forjo:'red', modern:'indigo', spa:'sage', cyber:'cyan' };
/* curated type pairings (no entry = each theme keeps its native fonts) */
const FONTS = [
  { id:'auto',       name:'Automática', meta:'Según estilo',     sample:'Aa', css:'var(--font-heading)' },
  { id:'geometrica', name:'Geométrica', meta:'Jakarta Sans',     sample:'Aa', css:"'Plus Jakarta Sans',sans-serif" },
  { id:'bauhaus',    name:'Bauhaus',    meta:'Grotesk · Archivo',sample:'Aa', css:"'Archivo',sans-serif" },
  { id:'elegante',   name:'Elegante',   meta:'Cormorant · Mulish',sample:'Aa', css:"'Cormorant Garamond',serif" },
  { id:'tech',       name:'Tech',       meta:'Orbitron · Chakra',sample:'Aa', css:"'Orbitron',sans-serif" },
  { id:'suave',      name:'Suave',      meta:'Sora · Manrope',   sample:'Aa', css:"'Sora',sans-serif" },
];
...
/* ---------- palette cards + dots (theme-aware, re-rendered on theme change) ---------- */
const palWrap = document.getElementById('palette-cards');
const dotsWrap = document.getElementById('pdots');
function renderPalettes(theme){
  const list = THEME_PALETTES[theme] || [];
  dotsWrap.innerHTML = list.map(p=>`
    <button class="pdot" data-p="${p.id}" title="${p.name}" style="background:${p.sw[0]}${p.glow?`;box-shadow:0 0 7px ${p.sw[0]}`:''}"></button>`).join('');
  palWrap.innerHTML = list.map(p=>`
...
function setPalette(p){
  ...
  const theme = localStorage.getItem('forjo-theme') || 'forjo';
  localStorage.setItem('forjo-pal-'+theme, p);
...
const themeWrap = document.getElementById('theme-cards');
themeWrap.innerHTML = THEMES.map(t=>`
  <button class="tcard" data-theme="${t.id}">
    <div class="tprev" style="background:${t.bg}">
      <span class="ttitle" style="color:${t.fg}">Aa</span>
      ${t.chips.map((c,i)=>`<span class="tchip" style="background:${c};${t.glow?`box-shadow:0 0 10px ${c}`:''};opacity:${1-i*0.18}"></span>`).join('')}
    </div>
    <div class="tbody">
      <div class="min0"><div class="tname truncate">${t.name}</div><div class="tmeta truncate">${t.meta}</div></div>
...
function setTheme(t){
  flushNoTransition();
  if(t==='forjo'){ delete root.dataset.theme; } else { root.dataset.theme = t; }
  localStorage.setItem('forjo-theme', t);
  document.querySelectorAll('#tsel .tsel-btn').forEach(b=>b.classList.toggle('active', b.dataset.theme===t));
  document.querySelectorAll('#theme-cards .tcard').forEach(c=>c.classList.toggle('sel', c.dataset.theme===t));
  /* rebuild the accent options for this theme, then apply its remembered (or default) accent */
  renderPalettes(t);
  setPalette(localStorage.getItem('forjo-pal-'+t) || THEME_DEFAULT_PAL[t]);
}
document.getElementById('tsel').addEventListener('click', e=>{ const b=e.target.closest('.tsel-btn'); if(b) setTheme(b.dataset.theme); });
themeWrap.addEventListener('click', e=>{ const b=e.target.closest('.tcard'); if(b) setTheme(b.dataset.theme); });

/* ---------- typography ---------- */
const fontWrap = document.getElementById('font-cards');
fontWrap.innerHTML = FONTS.map(f=>`
  <button class="fontcard" data-font="${f.id}">
    <span class="fprev" style="font-family:${f.css}">${f.sample}</span>
    <span class="min0"><span class="fname truncate">${f.name}</span><span class="fmeta truncate">${f.meta}</span></span>
    <span class="check">✓</span>
  </button>`).join('');
function setFont(f){
  flushNoTransition();
  if(f==='auto'){ delete root.dataset.font; } else { root.dataset.font = f; }
  localStorage.setItem('forjo-font', f);
  document.querySelectorAll('#font-cards .fontcard').forEach(c=>c.classList.toggle('sel', c.dataset.font===f));
}
fontWrap.addEventListener('click', e=>{ const b=e.target.closest('.fontcard'); if(b) setFont(b.dataset.font); });
...
const STANDALONE = ['reserva','confirmacion','login','onboarding'];
...
(function wireConfCopy(){
  const btn = document.getElementById('conf-copy'); if(!btn) return;
  btn.addEventListener('click', ()=>{
    const code = btn.closest('.conf-code').querySelector('.val').textContent.trim();
...
/* ---------- init from storage ---------- */
setTheme(localStorage.getItem('forjo-theme') || 'forjo');
setFont(localStorage.getItem('forjo-font') || 'auto');
