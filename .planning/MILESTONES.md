...
## v0.28 La agenda por servicio (Shipped: 2026-09-14)

**Phases completed:** 4 phases (18-21), 14 plans

**Key accomplishments:**

- **La franja dejó de saber solo *cuándo* y pasó a saber *qué* (Phase 18).** `time_blocks` podía decir "atiendo de 9 a 18" y no "a esta hora doy cerámica" — alcanzaba para una peluquería, no para un taller ni un estudio de danza, donde **la franja ES la clase**. Tabla puente `time_block_services` (migr. **071**) con la **regla del comodín**: **0 filas = la franja sirve para cualquier servicio**. Como eso ES el comportamiento vigente, el cutover fue **gratis y con cero regresión por construcción** — el día de la migración todos los negocios tenían 0 filas. Misma jugada que `professional_services` en v0.25 y que `individual` en v0.27.
- **Una sola implementación de la regla, y esta vez se verificó.** Vive en `lib/time-block-services.ts` y la consumen **siete** call sites: disponibilidad pública, backstop del motor, panel, aviso de borrado, booking público, onboarding y el editor de chips compartido. La auditoría de cierre buscó reimplementaciones y encontró **cero** — importa porque el modo de falla que AGENDA-02 existe para prevenir es exactamente que la regla se escriba dos veces y diverjan.
- **El inverso del comodín es más peligroso que el comodín.** `hasScheduleCoverage` devuelve `true` cuando el negocio no tiene ninguna franja ("la pregunta no tiene sujeto"), mientras la cruda `isServiceScheduled` devuelve `false` para todo. Cerrar los 7 días es **un click**, y con la función equivocada el aviso pasaba de informar a decirle al dueño que su catálogo entero se quedó sin horario. Mordió dos veces: CR-01 de la Phase 20 (`timeBlocks=[]` apagaba el catálogo completo) y otra vez en la Phase 21. Ningún consumidor de producción usa la cruda.
- **El dueño lo configura y el cliente ve la consecuencia (Phases 19-20).** Editor de chips por franja en `/agenda` con escritura **atómica** vía `save_agenda_blocks` (migr. **074**): franjas y mapeo entran o no entran juntos, así que "franjas sí, mapeo no" dejó de ser alcanzable. En público, el que elige un servicio ve **solo** los horarios donde se da, y uno sin cobertura queda deshabilitado **con el motivo a la vista** — distinguible del "sin profesional disponible", que es otro motivo.
- **El alta lo declara desde el día uno (Phase 21).** Un negocio de clases configura su agenda real en el onboarding sin tener que entrar al panel después. El editor de chips se **extrajo** a un componente compartido en vez de reimplementarlo, y la equivalencia visual con el panel se verificó en navegador. UAT **13/13**, cero issues.
- **El hallazgo del milestone no estaba en ningún plan, y se midió en vez de suponerse.** Preguntando "¿mandar el formulario vacío no perjudica al motor?" apareció que la invariante *"un servicio dura más de 0 minutos"* **no existía en la base**: vivía repetida en ~6 call sites de JavaScript. Y con `p_duration = 0` cada `tsrange(t, t+0)` es un **rango vacío**, que no solapa con nada: los **seis** gates anti-doble-booking pasaban en silencio. Reproducido contra Postgres: un turno confirmado de 60 min a las 14:00 y `book_slot_atomic(14:30, p_duration => 0)` **crea el turno encima, sin un solo error**. La trampa es que `COALESCE(x, 30)` atrapa el `NULL` pero **no el 0**. Migración **077**: normaliza, restringe con CHECK, y agrega un guard **fail-closed** en el RPC — no corrige a 30 en silencio, porque el default silencioso es el modo de falla contra el que se diseñó.
- **La UAT real volvió a encontrar lo que el pipeline no.** Los 4 gaps de la Phase 21 aparecieron tocando la pantalla con datos reales, ninguno lo veía un test: el botón "Cerrar sesión" pisando el logo, los campos numéricos que no se podían vaciar con el teclado, el desplegable de Rubro desbordando el viewport y los inputs de hora recortando el AM/PM — **este último sólo reproducía en un celular real**, porque en escritorio con locale de 24h el sufijo no existe. El de los inputs numéricos resultó **preexistente**, no una regresión: antes fallaba dando `NaN`.
- **La auditoría de cierre encontró lo que ninguna fase podía ver desde adentro.** Las cuatro estaban verdes en aislamiento; lo roto estaba en la costura: el **preview de la web del panel renderizaba el booking sin el mapeo**, así que el dueño veía todo habilitado y su cliente no. Investigándolo aparecieron dos divergencias más con la misma causa —el eje de staff y, peor, **un negocio de canchas viendo el booking genérico**, un componente distinto al que recibe su cliente—. Se cerró **eliminando el camino paralelo**: el preview arma `bookingSlot` como la página pública y esa prop pasó a ser **requerida**, así que un call site que olvide resolver el vertical **no compila**. Dejó de depender de disciplina.
- **Deploy y seguridad:** migraciones **071 → 077** aplicadas a mano en producción, en orden, con el código deployado **antes** de la 077 (la lección de la 068 en v0.27). Las cuatro fases SECURED con `threats_open: 0`. La **076** revocó `EXECUTE` sobre `book_slot_atomic` a `anon` — el riesgo alto preexistente que v0.27 dejó abierto. Suite 1163 tests, `tsc` limpio, cero dependencias nuevas.

**Known deferred items at close:** 3 warnings de la auditoría, ninguno incumpliendo un requirement. (1) **No hay aviso en el panel** para un servicio que ninguna franja cubre — y el camino más frecuente hacia el estado deshabilitado no es el alta, es agregar un servicio en `/settings` **después** de restringir franjas, mudo en las dos pantallas. (2) El **gate de canchas está implementado dos veces** (`canMapServicesInVertical()` y una comparación inline en el panel); hoy coinciden. (3) **`invalid_duration` no tiene mapeo a error de dominio**: cae en `insert_failed` 500 en vez de un 4xx útil — latente, los tres escritores de `services` están guardados. Además, dos cosas de infraestructura: la suite completa **flakea** intermitentemente en `test/abono-generation.test.ts` caso 5b (timeout de 5s que cascadea a 4 fallos; aislado da 24/24), y el **ledger de commits de los quick tasks se contamina entre tareas** porque la clave se deriva de `{phase}-{plan}` y todo quick de este repo es `quick-01`.

---

## v0.27 Cupo unificado por servicio (Shipped: 2026-08-24)

**Phases completed:** 3 phases (15-17), 17 plans

**Key accomplishments:**

- **El cupo pasa a vivir en un solo lugar (Phase 15).** `services.capacity_mode` es un enum de **tres** modos (`individual` default · `group_class` · `simultaneous_resource`) y `services.capacity` es la **fuente única** del número; `time_blocks.capacity` deja de decidir. "Individual" dejó de deducirse de otra tabla — un corte de pelo ya no queda etiquetado "Clase grupal". Migración **068** con backfill por predicado, dos CHECKs y el gate de cambio de modo que cierra el riesgo residual **R-1** de v0.26. El cutover **no afectó a nadie, y estaba medido**: los 19 bloques de producción tenían cupo máximo 1, así que el backfill —la parte que se estimaba cara— dejó de existir.
- **El gate hizo falta corregirlo dos veces más hasta que fue cierto.** El code review de la 15 encontró **3 blockers**, incluido un doble-booking real: la 068 volvió declarable con dos clicks una configuración —grupal con cupo ≥2— que **antes era inalcanzable**, y las tres protecciones que la 064 había construido filtraban por `capacity_mode = 'simultaneous_resource'` **literal**. Migración **069**. Y el fix que el review proponía **también estaba mal**, medido antes de aplicarlo: recortar por `capacity >= 2` reabría lo que cerró la 064, porque post-068 `capacity >= 2` ⟺ `modo <> individual`. El recorte correcto es **nominal Y numérico**.
- **Migración 070 (Phase 16): el gate deja de bloquear de más y de menos.** Se estrecha **por dirección** (`individual` → grupal/simultáneo es seguro y se bloqueaba sin motivo, justo en el cambio más frecuente porque `individual` es el default), marcar `completed` un turno futuro deja de abrirlo (residual R-15-A), y los dos gates comparan **fecha + hora** en vez de solo la fecha — el mismo bug que la Phase 13 arregló en la UI (gap G4) y que nunca había cruzado al SQL. El code review encontró después que espejar `isPastAppointment` era el error: **la UI pregunta "¿lo muestro como pasado?" y el gate pregunta "¿esto todavía ocupa la agenda?"**, y un turno EN CURSO responde distinto a cada una. Se corrigió a comparar contra el **fin** del turno.
- **El panel dejó de mentir (Phase 17).** El editor **explica** la diferencia entre grupal y simultáneo con el eje de conteo, un ejemplo y qué sale mal si elegís el otro; la tarjeta de `/servicios` muestra el modo **y** deja ajustar el cupo sin abrir nada (segundo write path, tenant-filtered, con copy propia del rechazo del gate); la grilla de la agenda calcula la ocupación desde `services.capacity` —la misma fuente que el motor— y Finanzas mobile muestra el servicio. 10 planes: 5 originales y 5 de cierre de gaps.
- **El hallazgo del milestone lo encontró el code review, no la UAT, y por un motivo preciso.** El panel agrupaba la ocupación por `date|time|service_id` y el motor cuenta por **agenda** (`COALESCE(professional_id, centinela)`), **sin `service_id`** — **ejes ortogonales**. Con dos profesionales dando la misma clase el panel mostraba `9/6` y decía "lleno" sobre un horario que la web seguía vendiendo. **Las cinco rondas de UAT no podían verlo: el fixture tenía un solo profesional, el único caso donde los dos ejes coinciden.** La distinción que faltaba —y que quedó escrita en el header del módulo— es **agrupar para MOSTRAR ≠ eje para CONTAR**. Los discriminantes se probaron **por mutación en direcciones opuestas**: ningún eje único deja verdes a los dos conjuntos de casos.
- **La UAT humana volvió a pagar el ciclo, cinco rondas seguidas.** Los 4 gaps que encontró fueron **todos de composición a 375px, ninguno de lógica** — y dos son invisibles para el pipeline. El mejor: **G-04 no se reproduce con mouse** — tocar el texto de la tarjeta movía el cupo por *tap-target fuzzing* contra botones de 44px a 4px de distancia; izquierda bajaba, derecha subía, y esa asimetría fue la prueba de que era geometría y no un handler. Se cerró con 32px de zona de exclusión. G-04 salió además de arreglar G-02b, que salió de un pedido del propio dueño: no fue error de ejecución sino una consecuencia que ni el UI-SPEC ni el plan previeron.
- **Un gate de presencia no ve un `id` duplicado.** `secure-phase` validó el `sr-only` del explicador de modos, pero `CapacityModeFields` **se monta dos veces a la vez** (alta + diálogo) y el `aria-describedby` del diálogo resolvía al bloque del formulario de alta. Lo cazó el code review; se arregló con `useId`.
- **Deploy y seguridad:** migraciones **068 → 070** aplicadas a mano en producción, en orden. ⚠ La 068 se aplicó **antes** de deployar el código y rompió la creación de servicios (el editor viejo insertaba `group_class` + cupo 1, que el CHECK nuevo rechaza) — se arregló **deployando, no con una migración correctiva**, que habría tenido que aflojar el CHECK. Las tres fases SECURED (32/32 · 19/19 · 54/54, `threats_open: 0`). Suite 26/26 en el módulo nuevo, `tsc` limpio, cero dependencias nuevas.

**Known deferred items at close:** 9 en `.planning/workstreams/motor-reservas/todos/pending/`. Los dos que importan son **pre-existentes y de seguridad**: `book_slot_atomic` es ejecutable por `anon` y saltea la ventana de reserva, el gate de plan y el reCAPTCHA —que viven solo en el route handler— (**severidad alta**, desde la migr. 041), y el filtro por tenant del gate se esquiva moviendo `services.business_id` a otro negocio propio (media). Además: dos clases grupales a la misma hora comparten el espacio de `seat` porque el índice único no incluye `service_id` · el roster dice "Seña pendiente" también cuando el hold venció · el editor de horarios sigue escribiendo `time_blocks.capacity`, jubilada por la 068 · las suites de abono son flaky en paralelo, así que `vitest run` completo **no sirve hoy como gate**.
