...
- **D-07:** El callback endurecido de Phase 4 (open-redirect cerrado por DESTINATIONS, token leak mitigado, cliente anon) se reusa **tal cual**. Phase 5 lo extiende así (CORREGIDO por el research — la redacción original era técnicamente incorrecta): OAuth vuelve con `?code=` y **sin** `type`, así que el route **branchea por presencia de `code`** → `exchangeCodeForSession`; el path `token_hash`/`verifyOtp` de Phase 4 queda intacto para el else. `oauth` **NO** entra en `ALLOWED_TYPES` (ese Set es para el `type` de los links OTP; un `type=oauth` fabricado iría a `verifyOtp` — contraproducente). Solo se suma la fila `oauth: '/dashboard'` a `DESTINATIONS`. Sin tocar los pasos 1, 3 ni 4 en lo demás.
- **D-08:** Rama de error OAuth propia (gap que encontró el research sobre D-05): un fallo de Google (o la cancelación del consent, que vuelve con `?error=`) NO puede caer en `/forgot-password?error=invalid_link` (mensaje de reset, opaco acá). Destino de error propio, ej. `/login?error=oauth`, con mensaje claro.
- **D-09:** `DESTINATIONS.oauth = '/dashboard'` (NO `/onboarding`): el layout del dashboard ya redirige a `/onboarding` si no hay negocio, así que un solo destino sirve para el usuario nuevo (→onboarding) y el recurrente (→dashboard). Apuntar a `/onboarding` rompería al recurrente (AUTH-04 verificado en `app/(dashboard)/layout.tsx:23`).
- **D-10:** UAT **prod-first** con cuenta descartable nueva. El OAuth de Google contra el Supabase local exige credenciales reales de Google + `skip_nonce_check` + URI `127.0.0.1:54321` — no vale la pena reproducirlo en local. Se prueba en prod tras el deploy, como el paso E de Phase 4.
