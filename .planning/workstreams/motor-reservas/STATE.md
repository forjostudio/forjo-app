...
| 260828-pir | Cierra las 3 amenazas abiertas del audit de la fase 19. **Migr. 075** `tb_location_same_tenant`: UNIQUE en `locations(id,business_id)` + FK compuesta — `save_agenda_blocks` escribía `location_id` sin validar pertenencia (T-19-36/WR-04). ⚠ El SQL que proponía el audit en §4.1 estaba MAL: `time_blocks.business_id` es NULLABLE, así que `ON DELETE SET NULL` sin lista de columnas huerfanizaba la franja (medido: `business_id_quedo_null = t`); la forma correcta es `ON DELETE SET NULL ("location_id")`. Además: guard fail-closed extendido de 1 a 5 counts en el pre-check de borrado (T-19-39/WR-07), y contra-caso cross-tenant del 5º count con `seedTwoTenants` cuya mordida se EJECUTÓ (T-19-14). tsc limpio, `npm test` 1067 passed, eslint 11→11. ⚠ **075 SIN APLICAR A PROD** — aplicarla cierra T-19-36 y además T-19-32 (lleva el `NOTIFY pgrst` final); runbook en la cabecera del .sql | 2026-08-28 | 667e4dc | [260828-pir-cerrar-las-3-amenazas-abiertas-de-la-fas](./quick/260828-pir-cerrar-las-3-amenazas-abiertas-de-la-fas/) |
| 260831-di2 | Cierra los 3 hallazgos priorizados del audit visual 6-pilares (19-UI-REVIEW.md, 18/24). **BLOCKER**: "Duracion del turno" y "Descanso entre turnos" los persiste el mismo saveHours pero quedaban fuera de hoursDirty Y del congelamiento de CR-01 -> perdida silenciosa. Causa de raiz cerrada con estructura: los 6 sitios que leian slotDuration/bufferMinutes se colapsan en un objeto `hoursConfig` con un unico camino de escritura, asi que un campo nuevo ensucia solo. Ojo: **congelarse sigue siendo manual, control por control**. Ademas: setHoursDirty(false) corria ANTES del UPDATE (agujero que el audit no nombro) -> se re-arma en la rama bizError. **role=status**: live regions ahora montadas antes de tener contenido, via sr-only DESACOPLADA del chip visible (montar el contenedor visible habria corrido la fila 8px por el gap del flex, rompiendo el escenario #2 de la UAT ya aprobado). **Ver todos (N)** deja de contar el comodin. UI-SPEC enmendado (la fila Guardando pasa de enumeracion a REGLA: fue la enumeracion la que dejo afuera los 2 Select). tsc limpio, npm test 1067 passed, eslint 1->1. WARN: human-check del lector de pantalla PENDIENTE (no autoaprobado) | 2026-08-31 | d73c58b | [260831-di2-cerrar-los-3-hallazgos-del-audit-visual-](./quick/260831-di2-cerrar-los-3-hallazgos-del-audit-visual-/) |
| 260831-kxt | Agenda avisa antes de perder horarios sin guardar, por las DOS vías. Nació de un uso real en prod: el dueño cambió Duración del turno, se fue y lo perdió — el aviso pasivo de la fase 19 no alcanza porque vive a ~160 líneas de JSX de los controles que lo encienden, o sea fuera de la pantalla. **Navegación interna:** `onNavigate` del `<Link>` del App Router (VERIFICADO en Next 16.2.7 instalado: `client/app-dir/link.d.ts:170`) — elegido sobre click-en-captura, que obligaría a reimplementar modificadores/`target=_blank`/`download`. **Cerrar/recargar:** `beforeunload`, espejando el precedente ya en prod de `web-client.tsx`. Nuevo: `lib/unsaved-changes.ts` (decisión pura, TDD, 8 casos) + `components/dashboard/unsaved-changes-guard.tsx` (provider + diálogo). El `ConfirmDialog` del CRM se DESCARTÓ: exige `risk` y resuelve peligro vía `useShellScope()`/`--danger`, que no existen fuera de `.crm-shell`. Logout con continuación (un push a /login sin `signOut()` deja la sesión viva y el proxy rebota). tsc limpio, `npm test` 1075 passed, eslint sin nuevos. ⚠ LÍMITES: el botón ATRÁS del navegador NO queda cubierto (el App Router no expone API de bloqueo; el hack de `history.pushState` desincroniza el router) y la bandera es sucia POR GESTO, no por diff. ⚠ human-check de 14 puntos PENDIENTE, no auto-aprobado | 2026-08-31 | f43e0fe | [260831-kxt-avisar-antes-de-perder-cambios-sin-guard](./quick/260831-kxt-avisar-antes-de-perder-cambios-sin-guard/) |
| 260902-h6m | **Migr. 076** cierra el bypass anónimo de `book_slot_atomic` (hallazgo del secure-phase de la Phase 16, severidad ALTA, meses en el backlog). La función es `SECURITY DEFINER` con `GRANT EXECUTE` al rol anónimo desde la 041; adentro de una definer **la RLS no corre**, así que llamando al RPC directo se salteaban los TRES controles que viven sólo en el route handler: ventana de reserva, gate de plan y reCAPTCHA (los 2 parámetros no adivinables son públicos vía `public_services`). NO era cross-tenant. **Bypass REPRODUCIDO por dos vías** antes de escribir nada (anon key sin sesión + `SET LOCAL ROLE anon` con ROLLBACK: el mismo rol que NO puede hacer un INSERT directo sí creaba el turno por la función). Camino elegido = REVOCAR, no mover la ventana adentro: mover cierra 1 de 3 controles, revocar cierra los 3. Precondición verificada: **ningún caller usa la anon key** (booking público=service_role, alta manual=authenticated, abonos=**dos** invocadores, cron service_role + alta manual authenticated — el plan decía service_role uniforme, corregido al medir). Post-fix: anon=f, authenticated=t, service_role=t. `npm test` 1083 vs baseline 1080. **⚠ 076 SIN APLICAR A PROD — el bypass sigue abierto allá**; runbook en la cabecera del .sql, no acoplada a ningún deploy | 2026-09-02 | dee3c7a | [260902-h6m-cerrar-el-bypass-anon-de-book-slot-atomi](./quick/260902-h6m-cerrar-el-bypass-anon-de-book-slot-atomi/) |
| 260902-njf | Cierra los hallazgos P1/P2/P3 del audit de Impeccable sobre el **booking público** (score 13/20, registro *product*). **P1-A:** los 4 campos del form no estaban asociados a sus labels (0 `id`, 0 `htmlFor`; el `Label` del repo es un `<label>` pelado) — era **WCAG 1.3.1/3.3.2 nivel A** en el form que llena un cliente final; resuelto con `useId`. **P1-B:** flechas de mes (32px) y slots (36px) suben a 44; los **días del calendario quedan en ~40px A PROPÓSITO** — medido: 301px útiles a 375px, 7×44=308, **no entra ni con `gap-0`**; la aritmética y las 3 alternativas descartadas quedaron escritas EN EL ARCHIVO (WCAG 2.5.8 pide 24×24; los 44 son HIG, no obligación). **P2:** obligatorios/opcionales según la regla del propio CLAUDE.md, validación inline onBlur, `width`/`height`/`loading` en las 2 `<img>`. **P3:** 5 tamaños arbitrarios → `text-xs`, anillo de foco en los 11 botones custom. ⚠ El teléfono NO se apretó: piso de 8 dígitos tras quitar no-numéricos, no regex de formato — una regla estricta bloquearía una reserva legítima, peor que el hallazgo. tsc limpio, eslint 0 bytes, `npm test` 1083 sin caída. ⚠⚠ **NINGÚN test automatizado cubre `booking-client.tsx`** (la suite valida `lib/`) → el verde NO prueba que la reserva entre; **human-check PENDIENTE**, incluido probar un teléfono argentino real. Fuera de alcance y sin tocar: el `border-l-4` de marca (decisión pendiente del usuario) | 2026-09-02 | ca84691 | [260902-njf-cerrar-los-hallazgos-del-audit-del-booki](./quick/260902-njf-cerrar-los-hallazgos-del-audit-del-booki/) |
