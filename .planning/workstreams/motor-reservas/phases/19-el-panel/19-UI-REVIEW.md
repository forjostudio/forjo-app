...
| WARNING-01 — `Ver todos (N)` incluye el comodín | **FIXED** | `d73c58b` — el trigger pasa a `Ver todos (${shown.length})`. `total` **no se toca** y sigue siendo lo único que decide `collapsible`: que el comodín cuente para el **umbral** es correcto y lo pide el spec; lo que cambia es sólo el número que se le muestra al dueño. |
