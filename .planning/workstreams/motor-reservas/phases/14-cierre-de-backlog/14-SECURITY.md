...
| R-3 | ~~La remediación no tuvo UAT visual.~~ **CERRADO — verificado por el dueño el 2026-08-11**, con la paleta **verde** activa (la prueba que importa: con la paleta por defecto el primario ya es rojizo y el defecto sería indistinguible). Observado en `/servicios` sobre un servicio sin turnos futuros —el único estado donde el confirm destructivo se dibuja—: botón **outline con borde rojo**, distinto del chip "Alto" relleno. Confirmado también que en estado **bloqueado** el "Desactivar" promovido a primario sale del color de la marca, que es correcto: es la acción segura, no la destructiva. | closed |
