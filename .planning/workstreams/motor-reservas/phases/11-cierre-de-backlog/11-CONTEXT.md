...
- **D-05:** setting **por negocio** que define la prominencia de "Cualquiera" en el paso "Profesional" de la reserva pública. **Hallazgo del research (RESEARCH A1, resuelto):** el selector SIEMPRE exige un tap (cada tarjeta llama `setStep(3)`), así que "preseleccionar" es inerte — la única palanca observable es el **ORDEN/prominencia de la tarjeta "Cualquiera"** (Opción A, confirmada por el usuario). Con **'any'** la tarjeta "Cualquiera" va **arriba de todo** (como hoy). Con **'choose'** va **debajo de la lista de profesionales** (profesionales primero). En ambos casos "Cualquiera" se sigue mostrando (ASIGN-01 intacto). NO es "preselección" ni ocultar la opción.
