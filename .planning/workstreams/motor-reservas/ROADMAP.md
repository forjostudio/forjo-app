...
### Milestone v0.29 — El catálogo del booking (en curso)
...
> El detalle de las Phases 18-21 vive en `.planning/milestones/v0.28-ROADMAP.md`.
> Se saca de acá para que el roadmap vivo no crezca un milestone por vez.
...
### Phase 22: El modelo del catálogo

**Goal**: Que el catálogo de un negocio pueda **declarar cómo se lee** —categorías propias, con su orden y su modo de orden— sin que eso cambie **nada** para quien no declare nada. El modelo es una tabla `service_categories` por negocio (migr. **078**, la única del milestone) más `services.category_id` **nullable**, y la regla es la del comodín un escalón más abajo que en v0.25 y v0.28: **la ausencia de dato significa "se muestra como hoy", nunca "no se muestra"**. Eso es lo que hace **CAT-07 verdadero POR CONSTRUCCIÓN** —cero categorías ⇒ la lista plana de hoy— y es la promesa que no se puede romper, porque el día de la migración **todos** los negocios de producción tienen cero categorías: el camino sin categorías es el de **todos** los clientes actuales. El orden vive en columnas (`sort_order`) y el **modo** de orden vive en el negocio (categorías: alfabético · personalizado; servicios: alfabético · precio · personalizado): el modo elige un **comparador de display** dentro de una función **pura**, que por definición no puede escribir — de ahí sale CAT-06 sin código extra, igual que el comodín salía de la forma de la tabla puente en v0.28. La regla de agrupar y ordenar vive en **un solo módulo puro con tests** (molde `lib/staff-services.ts` y `lib/time-block-services.ts`), nunca reimplementada por sus consumidores, y el público se alimenta de una **vista acotada** (`public_service_categories`) en vez de la tabla.
**Depends on**: Phase 21 (secuencial dentro del workstream; funcionalmente **no** depende de v0.28 — el catálogo es territorio nuevo: no toca el motor, ni los constraints, ni la disponibilidad)
**Requirements**: CAT-06, CAT-07
**Success Criteria** (what must be TRUE):

  1. Un negocio con **cero categorías** produce la lista de hoy: **plana, sin títulos y sin "Otros"**, y con el mismo orden que hoy — hoy la lectura pública **no tiene `ORDER BY`** (`app/[slug]/page.tsx:80`), así que los defaults del modelo tienen que **reproducir** eso y no "mejorarlo" por sorpresa. Demostrado con **control negativo**, no argumentado (CAT-07).
  2. El arreglo manual **no se puede perder al cambiar de modo**: el orden vive en columnas y el modo sólo elige un comparador dentro de una función **pura**, que no escribe. Volver a personalizado devuelve el arreglo del dueño intacto, con un test que lo muerde (CAT-06).
  3. La regla de agrupar y ordenar —incluido **"Otros" al final**— está en **un solo módulo puro con tests**; ningún consumidor la reimplementa, y el default ante el dato ausente es la **lista plana**, nunca un catálogo vacío (el modo de falla que ya mordió dos veces en este repo: CR-01 de la Phase 20, y otra vez en la 21).
  4. Un servicio sin categoría y un negocio sin categorías son **estados válidos y permanentes** del modelo: `category_id` es nullable y **borrar una categoría no borra ni desactiva un solo servicio** (`ON DELETE SET NULL`) — organizar el catálogo nunca puede sacar algo de la venta.
  5. El anon lee las categorías por la **vista acotada** `public_service_categories` y la tabla base **no tiene policy `anon`**; el aislamiento por tenant se sostiene en la base, con `services.category_id` apuntando sólo a una categoría **del mismo negocio** (FK compuesta, molde de las migr. 073/075), probado con un caso cross-tenant.
...
**Goal**: Que el dueño pueda **usar** lo que la Phase 22 volvió declarable, en la pantalla donde ya administra sus servicios. Crea, renombra y borra categorías; le asigna **una o ninguna** a cada servicio —y asignar **no es obligatorio en ningún punto del flujo**, que es lo que mantiene vivo el camino de CAT-07—; las reordena **arrastrando y con ▲/▼**; y elige, para todo el negocio, cómo se ordenan las categorías y cómo se ordenan los servicios dentro de cada una. Las flechas **no son un extra**: son lo que hace que reordenar funcione en mobile y con teclado, donde `draggable` no existe. Y cuando el modo **no** es personalizado, los controles de reordenar **desaparecen**: un arrastre que no hace nada se lee como roto. Entra acá también la **descripción corta** (CAT-11), porque lo que falta es exactamente un campo de panel: `services.description` ya existe, `public_services` ya la expone y la tarjeta del booking ya la renderiza recortada a dos líneas (`booking-client.tsx:610`) — hoy es una **columna muerta** que sólo se puede escribir por SQL.
**Depends on**: Phase 22 (sin el modelo, la vista y el módulo puro no hay qué configurar ni con qué ordenar)
**Requirements**: CAT-01, CAT-02, CAT-03, CAT-04, CAT-05, CAT-11
**Success Criteria** (what must be TRUE):

  1. El dueño crea, renombra y borra categorías desde el panel, y el duplicado **sin distinguir mayúsculas** (`"Cortes"` / `"cortes"`) lo rechaza **la base**, con copy propia en pantalla — nunca el texto crudo del error (CAT-01).
  2. El dueño le asigna a cada servicio **una** categoría o ninguna, y no hay ningún punto del flujo donde asignar sea obligatorio (CAT-02).
  3. El dueño reordena categorías **arrastrando y con ▲/▼**; las flechas funcionan con teclado y en mobile, y el orden se persiste **renumerando la lista completa de hermanas** — nunca con swaps sueltos, que dejan huecos y empates (CAT-03).
  4. El dueño elige los dos modos de orden para **todo el negocio** (categorías: alfabético · personalizado; servicios: alfabético · precio · personalizado), y con un modo que no es personalizado los controles de reordenar **no se muestran** (CAT-04, CAT-05).
  5. El dueño escribe la descripción corta de un servicio con **tope de 120 caracteres y contador a la vista**, y lo que escribe es exactamente lo que la tarjeta del booking ya muestra hoy debajo del nombre (CAT-11).
...
**Goal**: Que la organización que el dueño le dio a su catálogo **llegue al cliente**, y que la grilla deje de pelearse con el ancho en desktop. Con al menos una categoría creada, los servicios se ven **agrupados bajo títulos** en el orden que el dueño definió, con los que no tienen categoría **al final bajo "Otros"** —nunca escondidos: un servicio sin categoría siempre se puede reservar—; y con **cero categorías** la pantalla es la de hoy. El funnel **no gana pasos**: sigue siendo elegir servicio → profesional → día → horario. En la misma pasada, las tarjetas de desktop pasan a ser **horizontales a lo ancho**, el mismo formato que en mobile ya se ve mejor, y el nombre largo deja de partirse en dos líneas.
**Depends on**: Phase 22 (el módulo puro y la vista acotada) y Phase 23 (tiene que haber categorías creadas **desde el panel** para verificar el agrupado con datos reales, no sembrados por SQL)
**Requirements**: CAT-08, CAT-09, CAT-10
**Success Criteria** (what must be TRUE):

  1. Con al menos una categoría creada, el cliente ve los servicios **agrupados bajo títulos**, en el orden que el dueño definió, y el wizard **sigue teniendo 4 pasos** (CAT-08).
  2. Un servicio **sin categoría** se puede reservar siempre: con categorías creadas aparece **al final bajo "Otros"**, y nunca desaparece del catálogo público (CAT-09).
  3. Con **cero categorías** la pantalla es la de hoy —sin títulos, sin "Otros"—, verificado **end-to-end en el navegador** y no sólo en el módulo puro (CAT-07 lo garantizó la Phase 22 por construcción; acá se comprueba sobre la pantalla real).
  4. En desktop las tarjetas son **horizontales a lo ancho** y el nombre largo deja de partirse en dos líneas; en mobile **no cambia nada**, y la descripción sigue entrando en las dos líneas del `line-clamp-2` a 375px — que es justo lo que el tope de 120 caracteres de CAT-11 calibra (CAT-10).
  5. Los **otros dos call sites** del mismo componente —el preview de `/web` y el `LandingRenderer`— siguen mostrando el catálogo aunque no reciban las categorías: el default ante el dato ausente es la lista plana, nunca un catálogo vacío (el molde del `?? []` que ya usa `timeBlockServices` en `:566`).
...
| 22. El modelo del catálogo | 0/TBD | Not started | - |
