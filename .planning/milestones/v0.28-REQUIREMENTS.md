# Requisitos archivados: v0.28 — La agenda por servicio

> Workstream `motor-reservas`, **Phases 18-21**. Shipped **2026-09-14**.
> Roadmap archivado: `.planning/milestones/v0.28-ROADMAP.md`.
> Auditoría de cierre: `.planning/workstreams/motor-reservas/v0.28-MILESTONE-AUDIT.md`.

## Resultado

**8/8 satisfechos.** La auditoría de cierre los verificó **contra el código**, no heredando la tabla
de traceability: trazó los flujos end-to-end de un negocio con franjas genéricas (AGENDA-04, la
promesa de cero regresión) y de uno de clases declarando su agenda en el alta (AGENDA-08).

| Req | Fase | Cómo se cerró |
|-----|------|----------------|
| AGENDA-01 | 18 | Tabla puente `time_block_services` (migr. 071) con la regla del comodín |
| AGENDA-02 | 18 | `lib/time-block-services.ts`; la auditoría verificó **cero reimplementaciones** en los 7 call sites |
| AGENDA-03 | 18 | `/api/booking/availability` vía `startTimesNotOffered` (resta de conjuntos, no filtro) |
| AGENDA-04 | 18 | Cero regresión **por construcción**: 0 filas el día de la migración. Respuesta byte-idéntica verificada |
| AGENDA-05 | 19 | Editor de chips por franja en `/agenda`, escritura atómica vía `save_agenda_blocks` (migr. 074) |
| AGENDA-06 | 19 | Una franja sin servicios se lee "Cualquier servicio"; la ausencia **es** la regla, no un dato faltante |
| AGENDA-07 | 20 | Servicio sin cobertura deshabilitado **con el motivo**, distinguible del "sin profesional" |
| AGENDA-08 | 21 | Paso Horarios del alta con el toggle y el aviso de servicios sin franja; UAT 13/13 |

---

_Lo que sigue es el archivo de requisitos tal como se escribió antes de ejecutar el milestone._
